...
let busquedaDocumentoSeccion = document.querySelector(".busquedaDocumentoSeccion");
...
function buscarCitasCliente() {
    let urlBusquedaCitas = urlDomain + "/appointment/client/" + documentoABuscar.value;



        axios.get(urlBusquedaCitas)
            .then(response => {
                if (response.status === 200) {
...
function actualizarCita(idCita) {
    ...
    if (!busquedaDocumentoSeccion.classList.contains("d-none")) {
        busquedaDocumentoSeccion.classList.add("d-none");
    }
    ...
    await axios.put(urlModificar, datosNuevos)
        .then(response => {
            if (response.status === 200) {
                ...
                idCitaAModifica = "";
                if (busquedaDocumentoSeccion.classList.contains("d-none")) {
                    busquedaDocumentoSeccion.classList.remove("d-none");
                }
        ...
        .catch(error => {
            console.error(error)
            alertaError("No se pudo modificar la cita.");
        })
}
